...
FF_LIBS="libavcodec libavdevice libavfilter libavformat libavutil libswscale libswresample"
do_lipo_ffmpeg () {
    LIB_FILE=$1
    LIPO_FLAGS=
...
if [ "$FF_TARGET" = "arm64" ]; then
    echo_archs
    sh compiler.sh $FF_TARGET $FF_TARGET_EXTRA
    do_lipo_all
    ...
    for ARCH in $FF_ALL_ARCHS
    do
        sh compiler.sh $ARCH $FF_TARGET_EXTRA
    done
    ...
    do
        ...
        echo "=================="
        cd arch/ffmpeg-$ARCH && git clean -xdf && cd -
    done
